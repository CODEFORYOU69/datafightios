...
class ActionPickerView: UIView {
    ...
    private func requestChronoTime() {
        ...
        slider.addTarget(
            self, action: #selector(sliderValueChanged(_:)), for: .valueChanged)
        ...
        confirmButton.addTarget(
            self, action: #selector(confirmChronoTime), for: .touchUpInside)
    ...
    private var previousSelections:
        [(category: ActionCategory, action: Action)] = []
    ...
    init(
        frame: CGRect, initialAction: Action, points: Int, isIVRRequest: Bool,
        round: Round, onComplete: @escaping (Action?) -> Void,
        onCancel: @escaping () -> Void, onUndo: @escaping () -> Void
    ) {
    ...
    private func setupView() {
        ...
        undoButton.addTarget(
            self, action: #selector(undoButtonTapped), for: .touchUpInside)
    ...
    private func addButtons(
        options: [String], actionHandler: @escaping (Int) -> Void
    ) {
    ...
    private func updateForCurrentCategory() {
        ...
        case .zone:
            ...
            addButtons(options: Zone.allCases.map { $0.rawValue }) {
        ...
        case .actionType:
            ...
            addButtons(options: ActionType.allCases.map { $0.rawValue }) {
                [weak self] selectedIndex in
        ...
        case .impactArea:
            ...
            addButtons(options: ["Head (3 points)", "Chest (2 points)"]) {
                [weak self] selectedIndex in
        ...
        case .selectFighter:
            ...
            addButtons(options: ["Blue Fighter", "Red Fighter"]) {
                [weak self] selectedIndex in
                self?.action.color = selectedIndex == 0 ? .blue : .red
                self?.action.fighterId =
                    selectedIndex == 0
                    ? self?.action.blueFighterId ?? ""
                    : self?.action.redFighterId ?? ""
                self?.moveToNextCategory()
            }
        ...
        case .technique:
            ...
            let techniques = Technique.allCases.filter {
                $0.points == self.points
            }
            addButtons(options: techniques.map { $0.rawValue }) {
                [weak self] selectedIndex in
        ...
        case .limb:
            ...
            addButtons(options: limbs.map { $0.rawValue }) {
                [weak self] selectedIndex in
        ...
        case .guardPosition:
            ...
            addButtons(options: GuardPosition.allCases.map { $0.rawValue }) {
                [weak self] selectedIndex in
                self?.action.guardPosition =
                    GuardPosition.allCases[selectedIndex]
                self?.moveToNextCategory()
            }

        case .gamjeonType:
            titleLabel.text = "Gamjeon Type"
            addButtons(options: GamjeonType.allCases.map { $0.rawValue }) {
                [weak self] selectedIndex in
        ...
        case .situation:
            ...
            addButtons(options: CombatSituation.allCases.map { $0.rawValue }) {
                [weak self] selectedIndex in
