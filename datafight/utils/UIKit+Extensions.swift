...
struct StyleGuide {
    ...
    // MARK: - Colors
    struct Colors {
        static let primaryBackground = UIColor(red: 0.98, green: 0.98, blue: 1.0, alpha: 1.0)
        static let secondaryBackground = UIColor.white
        static let accentColor = UIColor(red: 0.4, green: 0.6, blue: 1.0, alpha: 1.0)
        static let textColor = UIColor(red: 0.2, green: 0.2, blue: 0.2, alpha: 1.0)
        static let subtleText = UIColor(red: 0.5, green: 0.5, blue: 0.5, alpha: 1.0)
        static let borderColor = UIColor(red: 0.9, green: 0.9, blue: 0.9, alpha: 1.0)
    }
    ...
    static func applyButtonStyle(to button: UIButton) {
        ...
        gradient.colors = [Colors.accentColor.cgColor, Colors.accentColor.withAlphaComponent(0.8).cgColor]
        gradient.locations = [0.0, 1.0]
        gradient.frame = button.bounds
        button.layer.insertSublayer(gradient, at: 0)
        
        // Haptic feedback
        button.addTarget(button, action: #selector(UIButton.buttonTapped), for: .touchUpInside)
    }
    ...
    static func applyTextFieldStyle(to textField: UITextField) {
        ...
        innerShadow.shadowPath = UIBezierPath(roundedRect: innerShadow.bounds.insetBy(dx: 2, dy: 2), cornerRadius: Dimensions.standardCornerRadius).cgPath
    ...
    static func addIconToTextField(_ textField: UITextField, iconName: String) {
        let iconView = UIImageView(frame: CGRect(x: 10, y: 5, width: Dimensions.iconSize, height: Dimensions.iconSize))
    ...
    static func applyBackgroundGradient(to view: UIView) {
        ...
        gradientLayer.colors = [Colors.primaryBackground.cgColor, Colors.secondaryBackground.cgColor]
