...
class AddRoundViewController: UIViewController {
        ...
        FirebaseService.shared.getLastRoundEndTime(for: fight) { [weak self] result in
            ...
            self.requestNewRoundStartTime(lastEndTime: lastEndTime) { newStartTime in
                self.currentRound = Round(
                    ...
                    chronoDuration: 0,  // Sera défini plus tard lorsque la vidéo est chargée
                    ...
                    victoryDecision: nil,
                    roundWinner: nil,
                    startTime: newStartTime  // Nouveau champ pour le temps de début
                )
    ...
    func requestNewRoundStartTime(lastEndTime: TimeInterval, completion: @escaping (TimeInterval) -> Void) {
        let alertController = UIAlertController(title: "New Round Start Time", message: "Enter the start time for the new round (in seconds):\nLast round ended at: \(lastEndTime) seconds", preferredStyle: .alert)
