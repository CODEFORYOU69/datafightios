...
class EventEditViewController: UIViewController {
    ...
    let eventTypes = ["Open", "Regional Championship", "National Championship", "Continental Championship", "World Championship", "G1", "G2", "Grand Slam", "Grand Prix"]
    ...
    func setupUI() {
        if let event = event {
            ...
            if let index = eventTypes.firstIndex(of: event.eventType) {
    ...
    @objc func saveTapped(_ sender: Any) {
        ...
        let eventType = eventTypes[eventTypePicker.selectedRow(inComponent: 0)]
