...
extension AddRoundViewController {
    ...
    func endRound() {
        ...
        if currentRoundNumber > MAX_ROUNDS {
            ...
            showAlert(
                title: "Error",
                message: "Maximum number of rounds (3) has been reached.")
            return
        }
        ...
        requestRoundEndReason { [weak self] isEndedByTime in
            ...
            self.requestHitsCount { blueHits, redHits in
                ...
                let (winner, decision) = updatedRound.determineWinner(
                    isEndedByTime: isEndedByTime, determiner: self)
                print("Initial decision: \(String(describing: winner))")

                self.finalizeRoundDecision(
                    currentRound: updatedRound, fight: fight, winner: winner,
                    decision: decision
                ) { finalizedRound in
                    self.currentRound = finalizedRound
                    self.showRoundSummary(
                        round: finalizedRound,
                        winner: FighterColor(
                            rawValue: finalizedRound.roundWinner
                                == fight.blueFighterId ? "blue" : "red")
                    ) {
                        self.saveRoundToFirebase(
                            finalizedRound, endTimestamp: endTimestamp
                        ) {
    ...
    func finalizeRoundDecision(
        currentRound: Round, fight: Fight, winner: FighterColor?,
        decision: VictoryDecision, completion: @escaping (Round) -> Void
    ) {
        var localRound = currentRound
        ...
        if decision == .referee {
            ...
            self.handleRefereeDecision { refereeDecision in
                ...
                localRound.roundWinner =
                    refereeDecision == .blue
            ...
            print(
                "Winner determined: \(winner?.rawValue ?? "None"), Decision: \(decision)"
            )
            localRound.roundWinner =
                winner?.rawValue == "blue"
                ? fight.blueFighterId : fight.redFighterId
            localRound.victoryDecision = decision
            localRound.creatorUserId = currentUserId ?? ""
            completion(localRound)
        }
    }
    func handleRefereeDecision(completion: @escaping (FighterColor) -> Void) {
        DispatchQueue.main.async {
            ...
            let alert = UIAlertController(
                ...
                message:
                    "Please select the winner as determined by the referees",
                preferredStyle: .alert)

            let blueAction = UIAlertAction(
                title: "Blue Fighter", style: .default
            ) { _ in
                print("Blue Fighter selected")
                completion(.blue)
            }

            let redAction = UIAlertAction(title: "Red Fighter", style: .default)
            { _ in
    ...
    private func finalizeRound(
        _ round: Round, with decision: VictoryDecision,
        ...
        saveRoundToFirebase(updatedRound, endTimestamp: endTimestamp) {
            [weak self] in
    ...
    func prepareNextRound(previousEndTimestamp: TimeInterval) {
        ...
        if nextRoundNumber > MAX_ROUNDS {
            ...
            showAlert(
                title: "Fight Ended",
                message: "All rounds completed. Please finalize the fight.")
            return
        }
        ...
        requestPauseDuration { [weak self] pauseDuration in
            ...
            let newRound = Round(
                ...
                chronoDuration: self.videoPlayerView.player?.currentItem?
                    .duration.seconds ?? 0,
                duration: 0,
            ...
            self.updateVideoTimestamps(
                startTimestamp: newStartTime, endTimestamp: nil,
                roundNumber: nextRoundNumber)

            self.seekVideo(
                to: CMTime(seconds: newStartTime, preferredTimescale: 600))
            ...
            let alertController = UIAlertController(
                title: "New Round",
                message: "Round \(nextRoundNumber) is ready to start.",
                preferredStyle: .alert)
    ...
    func requestPauseDuration(completion: @escaping (Double) -> Void) {
        ...
        let alertController = UIAlertController(
            ...
            message: "Enter the pause duration (in seconds):",
            preferredStyle: .alert)
        ...
        let confirmAction = UIAlertAction(title: "OK", style: .default) { _ in
            let pauseDuration =
                Double(alertController.textFields?.first?.text ?? "0") ?? 0
    ...
    func requestRoundEndReason(completion: @escaping (Bool) -> Void) {
        ...
        let alertController = UIAlertController(
            title: "End of Round",
            message: "Did the round end by time expiration?",
            preferredStyle: .alert)
    ...
    func requestHitsCount(completion: @escaping (Int, Int) -> Void) {
        ...
        let alertController = UIAlertController(
            title: "Hits Count",
            message: "Enter the number of hits for each fighter",
            preferredStyle: .alert)
    ...
    func saveRoundToFirebase(
        _ round: Round, endTimestamp: TimeInterval,
        ...
        var updatedRound = round
        print("Round saved successfully with ID: \(updatedRound)")

        updatedRound.endTime = endTimestamp
        updatedRound.chronoDuration = chronoDuration

        FirebaseService.shared.saveRoundAndUpdateFight(updatedRound, for: fight)
        { [weak self] result in
            switch result {
            case .success(let roundId):
                ...
                self?.updateVideoTimestamps(
                    startTimestamp: updatedRound.startTime ?? 0,
    ...
    func updateVideoTimestamps(
        startTimestamp: TimeInterval, endTimestamp: TimeInterval?,
        roundNumber: Int
    ) {
        print(
            "Updating video timestamps: start - \(startTimestamp), end - \(String(describing: endTimestamp)), round - \(roundNumber)"
        )
        ...
        FirebaseService.shared.getVideo(by: videoId) { [weak self] result in
            ...
            case .success(var video):
                ...
                FirebaseService.shared.updateVideo(video) { updateResult in
                    ...
                    case .failure(let error):
                        print(
                            "Failed to update video timestamps: \(error.localizedDescription)"
                        )
                        self?.showAlert(
                            title: "Error",
                            message:
            ...
            case .failure(let error):
                ...
                self?.showAlert(
                    title: "Error",
                    message:
                        "Failed to retrieve video: \(error.localizedDescription)"
                )
            }
        }
    }
    ...
    func showFightCompletionAlert(result: FightResult) {
        ...
        let message =
            "Fight completed. Winner: \(result.winner), Method: \(result.method)"
        let alertController = UIAlertController(
            title: "Fight Completed", message: message, preferredStyle: .alert)
    ...
    func showFightCompletion(result: FightResult) {
        ...
        guard
            let fightResultVC = storyboard?.instantiateViewController(
                withIdentifier: "FightResultViewController")
                as? FightResultViewController,
            let fight = self.fight
        else {
            print(
                "Error: Unable to instantiate FightResultViewController or no fight available"
            )
            return
        }

        // Récupérer les fighters
        FirebaseService.shared.getFighter(id: fight.blueFighterId) {
            [weak self] blueResult in
            FirebaseService.shared.getFighter(id: fight.redFighterId) {
                [weak self] redResult in
                guard let self = self else { return }
                ...
                case (.success(let blueFighter), .success(let redFighter)):
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.present(
                            fightResultVC, animated: true, completion: nil)
                    }
                ...
                case (.failure(let blueError), _):
                    ...
                    self.showAlert(
                        title: "Error",
                        message: "Failed to retrieve blue fighter")

                case (_, .failure(let redError)):
                    print("Error retrieving red fighter: \(redError)")
                    self.showAlert(
                        title: "Error",
                        message: "Failed to retrieve red fighter")
                }
            }
        }
    }

    func showRoundSummary(
        round: Round, winner: FighterColor?, completion: @escaping () -> Void
    ) {
        print("Showing round summary")
        let winnerText =
            winner.map { $0.rawValue.capitalized }
            ?? "No winner (Referee decision)"
        let message = """
        ...
        let alertController = UIAlertController(
            title: "Round Summary", message: message, preferredStyle: .alert)
