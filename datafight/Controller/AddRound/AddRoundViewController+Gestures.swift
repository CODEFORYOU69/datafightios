...
extension AddRoundViewController {
    ...
    /// Sets up gestures for each zone (Z1, Z2, Z3), including taps and swipes.
    func setupZoneGestures() {
        ...
        for (index, zone) in zones.enumerated() {
            ...
            let tripleTapGesture = UITapGestureRecognizer(
                target: self, action: #selector(handleTripleTap(_:)))
            ...
            let tapGesture = UITapGestureRecognizer(
                target: self, action: #selector(handleZoneTap(_:)))
            ...
            for direction in directions {
                let swipeGesture = UISwipeGestureRecognizer(
                    target: self, action: #selector(handleZoneSwipe(_:)))
    ...
    /// Handles a double-tap gesture for Gamjeon in the given zone.
    ...
    /// Handles a single tap gesture in the given zone.
    ...
    /// Handles swipe gestures in the given zone, interpreting direction as different combat situations.
    ...
    /// Handles triple-tap gestures, typically for head kicks.
    ...
    /// Provides visual feedback for any gesture interaction by briefly dimming and restoring the view's opacity.
