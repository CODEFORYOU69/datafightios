...
func requestChronoTime(completion: @escaping (TimeInterval) -> Void) {
    let alertController = UIAlertController(title: "Enter Remaining Time", message: "Please enter the remaining time in the round (mm:ss)", preferredStyle: .alert)
    ...
    let confirmAction = UIAlertAction(title: "Confirm", style: .default) { _ in
        if let timeString = alertController.textFields?.first?.text, !timeString.isEmpty {
    ...
    func showActionWheel(for color: FighterColor, isIVRRequest: Bool) {
        requestChronoTime { [weak self] chronoTimestamp in
            ...
            showActionDetailsInterface(for: action, points: 0, isIVRRequest: isIVRRequest)  
        }
    }
    ...
    func removeGamjeonFromRequester(_ color: FighterColor) {
        ...
        if let activeGamJeons = currentRound?.actions.filter({ $0.actionType == .gamJeon && ($0.isActive != nil) }) {
        ...
        if let index = currentRound?.actions.lastIndex(where: { $0.color == color && $0.actionType == .gamJeon && ($0.isActive != nil) }) {
    ...
    func updateActionIcon(for action: Action) {
        ...
        if let iconView = IconActionRegistered.subviews.first(where: { $0.tag == action.id.hashValue }) {
            ...
            print("Updated icon for action: \(action.actionType), isActive: \(String(describing: action.isActive))")
        } else {
    ...
    func showRecentActionsForDeletion() {
        ...
        let recentActions = Array(currentRound?.actions.filter { $0.isActive ?? true }.suffix(3) ?? [])
