...
class FightDetailViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        Analytics.logEvent(
            ...
            parameters: [
                AnalyticsParameterScreenName: "Fight Detail"
            ])
    }
    ...
    private func setupBackground() {
        ...
        backgroundImageView.alpha = 0.2  // Lower opacity for subtle background
    ...
    func configureFightResult() {
        blueNameLabel.text =
            blueFighter?.fullName ?? fight?.blueFighterId ?? "Blue Fighter"
        redNameLabel.text =
            redFighter?.fullName ?? fight?.redFighterId ?? "Red Fighter"
        ...
        if let result = fightResult {
            let winnerName =
                ...
                ? blueNameLabel.text : redNameLabel.text
    ...
    private func createRoundResultView(for round: Round, fight: Fight?)
        -> UIView
    {
        ...
        let labels: [(String, String)] = [
            ...
            (
                "Winner: \(round.roundWinner == fight?.blueFighterId ? "Blue" : (round.roundWinner == fight?.redFighterId ? "Red" : "Unknown"))",
                "highlight"
            ),
    ...
    @IBAction func dismissTapped(_ sender: Any) {
        ...
        Analytics.logEvent("fight_detail_dismissed", parameters: nil)
    }
    ...
    @objc func respondToSwipeGesture(gesture: UIGestureRecognizer) {
        if let swipeGesture = gesture as? UISwipeGestureRecognizer {
            ...
            case .down:
                ...
                Analytics.logEvent("fight_detail_swiped_down", parameters: nil)
            default:
                break
            }
        }
