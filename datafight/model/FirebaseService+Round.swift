...
extension FirebaseService {
    ...
    func saveRound(_ round: Round, for fight: Fight, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        guard let fightId = fight.id else {
            ...
            completion(.failure(NSError(domain: "FirebaseService", code: 1, userInfo: [NSLocalizedDescriptionKey: "Fight ID is nil"])))
            return
        }

        let roundRef = db.collection("rounds").document(round.id ?? UUID().uuidString)
    ...
    func getRound(id: String, for fight: Fight, completion: @escaping (Result<Round, Error>) -> Void) {
        ...
        guard uid == fight.creatorUserId else {
            ...
            completion(.failure(NSError(domain: "FirebaseService", code: 1, userInfo: [NSLocalizedDescriptionKey: "User not authorized"])))
            return
        }
            ...
            if let error = error {
                ...
                do {
                    ...
                    guard round.fightId == fight.id else {
                        ...
                        completion(.failure(NSError(domain: "FirebaseService", code: 4, userInfo: [NSLocalizedDescriptionKey: "Round does not belong to this fight"])))
                        return
                    }
    ...
    func getRoundsForFight(_ fight: Fight, completion: @escaping (Result<[Round], Error>) -> Void) {
        ...
        guard uid == fight.creatorUserId else {
            ...
            completion(.failure(NSError(domain: "FirebaseService", code: 2, userInfo: [NSLocalizedDescriptionKey: "User does not have permission to access these rounds"])))
            return
        }
        ...
        db.collection("fights").document(fightId).getDocument { (document, error) in
            if let error = error {
                print("Failed to fetch fight document: \(error.localizedDescription)")
                completion(.failure(error))
                return
            }
            ...
            guard let document = document, document.exists else {
                ...
                completion(.failure(NSError(domain: "FirebaseService", code: 3, userInfo: [NSLocalizedDescriptionKey: "Fight not found"])))
                return
            }
            
            guard let roundIds = document.data()?["roundIds"] as? [String], !roundIds.isEmpty else {
            ...
            roundsRef.whereField(FieldPath.documentID(), in: roundIds).getDocuments { (snapshot, error) in
                if let error = error {
                ...
                if rounds.isEmpty {
                    ...
                    print("Successfully retrieved and decoded \(sortedRounds.count) rounds for fight \(fightId)")
    ...
    func updateRound(_ round: Round, for fight: Fight, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        guard let roundId = round.id else {
            completion(.failure(NSError(domain: "FirebaseService", code: 1, userInfo: [NSLocalizedDescriptionKey: "Round ID is missing"])))
            return
        }

        do {
                let roundData = try Firestore.Encoder().encode(round)
                db.collection("rounds").document(roundId).setData(roundData, merge: true) { error in
                if let error = error {
            ...
            print("Error converting round to dictionary: \(error.localizedDescription), \(error.userInfo)")
            completion(.failure(error))
        }
    }
    func deleteRound(id: String, for fight: Fight, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        db.collection("fights").document(fightId).collection("rounds").document(id).delete { error in
            if let error = error {
