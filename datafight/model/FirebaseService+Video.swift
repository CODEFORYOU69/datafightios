...
extension FirebaseService {
    ...
    func getVideo(
        by videoId: String, completion: @escaping (Result<Video, Error>) -> Void
    ) {
            ...
            completion(
                .failure(
                    NSError(
                        domain: "FirebaseService", code: 0,
                        userInfo: [
                            NSLocalizedDescriptionKey: "Video ID is empty"
                        ])))
            return
        }
    ...
    func updateVideoTimestamps(
        videoId: String, roundNumber: Int, startTimestamp: TimeInterval,
    ...
    func updateVideo(
        _ video: Video, completion: @escaping (Result<Void, Error>) -> Void
    ) {
    ...
    func updateVideoRoundTimestamps(
        for fight: Fight, roundNumber: Int, startTime: TimeInterval,
        completion: @escaping (Result<Void, Error>) -> Void
    ) {
        ...
        db.collection("videos").whereField("fightId", isEqualTo: fightId)
            .getDocuments { (querySnapshot, error) in
                ...
                document.reference.updateData([
                    ...
                    if let error = error {
                        print(
                            "Error updating round timestamps: \(error.localizedDescription)"
                        )
                        completion(.failure(error))
                    } else {
