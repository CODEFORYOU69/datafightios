...
extension FirebaseService {
    ...
    func fetchFilteredFights(
        ...
        guard let currentUserId = Auth.auth().currentUser?.uid else {
            ...
            throw NSError(
                domain: "FirebaseService", code: 401,
        ...
        var query: Query = db.collection("fights").whereField(
            "creatorUserId", isEqualTo: currentUserId)

        if let fighter = fighter {
            print("Ajout du filtre pour le combattant: \(fighter)")
            query = query.whereField("blueFighterId", isEqualTo: fighter)
                .whereField("redFighterId", isEqualTo: fighter)
        }
        ...
        if let weightCategory = weightCategory {
            print(
                "Ajout du filtre pour la catégorie de poids: \(weightCategory)")
            query = query.whereField(
                "weightCategory", isEqualTo: weightCategory)
        }
        ...
        let fights = querySnapshot.documents.compactMap { document -> Fight? in
            ...
            if let fight = fight {
                ...
                print(
                    "Impossible de convertir le document en Fight pour le document ID: \(document.documentID)"
                )
            }
            return fight
        }
        ...
        let filteredFights = fights.filter { fight in
            ...
            if let event = events.first(where: { $0.id == fight.eventId }) {
                print(
                    "Traitement de l'événement pour le combat ID: \(fight.id ?? "Inconnu")"
                )
                ...
                if let startDate = startDate, let endDate = endDate {
                    ...
                    print(
                        "Filtrage par date pour l'événement ID: \(event.id ?? "Inconnu"), inclure: \(include)"
                    )
                }
                ...
                if let eventType = eventType {
                    ...
                    print(
                        "Filtrage par type d'événement pour l'événement ID: \(event.id ?? "Inconnu"), inclure: \(include)"
                    )
                }
                ...
                if let eventCountry = eventCountry {
                    ...
                    print(
                        "Filtrage par pays pour l'événement ID: \(event.id ?? "Inconnu"), inclure: \(include)"
                    )
                }
            } else {
                print(
                    "Événement non trouvé pour le combat ID: \(fight.id ?? "Inconnu")"
                )
                include = false
            }
            ...
            if let fighterNationality = fighterNationality {
                let blueFighter = fighters.first(where: {
                    $0.id == fight.blueFighterId
                })
                let redFighter = fighters.first(where: {
                    $0.id == fight.redFighterId
                })

                let blueFighterMatch =
                    blueFighter?.country == fighterNationality
                let redFighterMatch = redFighter?.country == fighterNationality
                include = include && (blueFighterMatch || redFighterMatch)
                print(
                    "Filtrage par nationalité pour le combat ID: \(fight.id ?? "Inconnu"), inclure: \(include)"
                )
            }
    ...
    func fetchRounds(for fight: Fight) async throws -> [Round] {
        print(
            "Début de fetchRounds pour le combat ID: \(fight.id ?? "Inconnu")")

        guard let roundIds = fight.roundIds else {
            print(
                "Aucun roundId trouvé pour le combat ID: \(fight.id ?? "Inconnu")"
            )
            return []
        }
        ...
        return try await withThrowingTaskGroup(of: Round?.self) { group in
            for roundId in roundIds {
                group.addTask {
                    ...
                    let documentSnapshot = try await db.collection("rounds")
                        .document(roundId).getDocument()
                    let round = try? documentSnapshot.data(as: Round.self)
                    if let round = round {
                        print("Round récupéré: \(round)")
                    } else {
                        print(
                            "Impossible de convertir le document en Round pour le document ID: \(roundId)"
                        )
                    }
                    return round
                }
    ...
    func fetchEvents(
        ...
        eventsRef.whereField(FieldPath.documentID(), in: ids).getDocuments {
                ...
                completion(
                    .failure(
                        NSError(
                            ...
                            userInfo: [
                                NSLocalizedDescriptionKey: "No documents found"
                            ])))
                return
            }
    ...
    func fetchFighters(ids: [String]) async throws -> [Fighter] {
        ...
        let db = Firestore.firestore()
        let querySnapshot = try await db.collection("fighters").whereField(
            FieldPath.documentID(), in: ids
        ).getDocuments()
        print(
            "Nombre de documents combattants récupérés: \(querySnapshot.documents.count)"
        )

        let fighters = querySnapshot.documents.compactMap {
            document -> Fighter? in
            ...
            if let fighter = fighter {
                ...
                print(
                    "Impossible de convertir le document en Fighter pour le document ID: \(document.documentID)"
                )
            }
            return fighter
        }
