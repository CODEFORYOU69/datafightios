...
extension FirebaseService {
    // MARK: - Event Methods
    func updateEventWithFight(
        eventId: String, fightId: String,
        ...
        db.runTransaction({ (transaction, errorPointer) -> Any? in
            ...
            guard var event = try? eventDocument.data(as: Event.self) else {
                let error = NSError(
                    domain: "AppErrorDomain", code: -1,
                    userInfo: [
                        NSLocalizedDescriptionKey: "Unable to fetch event"
                    ])
    ...
    func saveEvent(
        _ event: Event, completion: @escaping (Result<String, Error>) -> Void
    ) {
    ...
    func getEvent(
        id: String, completion: @escaping (Result<Event, Error>) -> Void
    ) {
        guard let uid = Auth.auth().currentUser?.uid else {
            completion(
                .failure(
